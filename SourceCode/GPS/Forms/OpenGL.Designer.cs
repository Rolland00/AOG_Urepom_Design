...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
            ...
            else
            {
                ...
                for (int j = 0; j < tool.numOfSections; j++)
                {
                    ...
                    if (bnd.bndArr.Count > 0)
                    {
                        ...
                        {
                            if (alarm_headland == 0 && Properties.Settings.Default.Son_actif == 1)
                            {
        ...
        private void DrawUTurnBtn()
        {
            ...
            {
                GL.TexCoord2(0, 0); GL.Vertex2(-62 + two3, 70); // 
                GL.TexCoord2(1, 0); GL.Vertex2(62 + two3, 70.0); // 
        ...
        private void DrawSteerCircle()
        {
            ...
            //ajout max
            int bottomSide = oglMain.Height - 150;
            //fin
            GL.PushMatrix();
            GL.Enable(EnableCap.Texture2D);
        ...
        private void DrawLightBarText()
        {
            ...
            if (ct.isContourBtnOn || ABLine.isBtnABLineOn || curve.isBtnCurveOn)
            {
                ...
                if (dist != 3200 && dist != 3202)
                {
                    ...
                    font.DrawText(center, 50, hede, size);
                }
            }
            //if (ct.isContourBtnOn)
            //{
        ...
        private void DrawCompassText()
        {
            ...
            font.DrawText(center, 64, (fixHeading * 57.2957795).ToString("N1"), 0.7);

            if (isCompassOn && ( ahrs.isHeadingCorrectionFromBrick | ahrs.isHeadingCorrectionFromAutoSteer))
            {
                font.DrawText(center, 84, "G:"+(gpsHeading * 57.2957795).ToString("N1"), 0.7);

                font.DrawText(center, 104, "I:" + Math.Round(ahrs.correctionHeadingX16 * 0.0625, 1).ToString(), 0.7);
            }
        ...
        private void DrawLiftIndicator()
        {
            ...
            //ajout max
            GL.Translate(btnHydLift.Right + 40 - oglMain.Width / 2, round_table7.Top + 40, 0);
