...
namespace AgOpenGPS
{
    public partial class FormGPS
    {
        ...
        //ajout max
        public bool isUTurnAlwaysOn, isCompassOn, isSpeedoOn, isAutoDayNight, isSectionButtons, isUturn_without_Autosteer, isSideGuideLines = true;
        //fin
        public bool isPureDisplayOn = true, isSkyOn = true, isRollMeterOn = false;
        public bool isDay = false, isDayTime = true, isSimple;
            ...
            if (ScanForNMEA())
            {
                ...
                if (displayUpdateThreeSecondCounter != threeSeconds)
                {
                    ...
                    if (isMetric)
                    {
                        if (isJobStarted)
                        {
                            label3.Text = currentFieldDirectory.Substring(0, currentFieldDirectory.Length - 17) + "- " + fd.AreaBoundaryLessInnersHectares + " ha";
                        }
                        lblTotalAppliedArea.Text = fd.WorkedHectares + " ha";
                        lblWorkRemaining.Text = fd.WorkedAreaRemainHectares + " ha";            
                        lblTimeRemaining.Text = fd.TimeTillFinished + " Hr";
                        ...
                        label2.Text = toolFileName + (Math.Round(tool.toolWidth, 2)).ToString() + " m";
                        //ajout max
                ...
                if (displayUpdateOneSecondCounter != oneSecond)
                {
                    ...
                    if (mc.isOutOfBounds && panel_top.BackColor == Color.Transparent
                        || !mc.isOutOfBounds && panel_top.BackColor == Color.Tomato)
                    {
                        if (!mc.isOutOfBounds)
                        {
        ...
        private void LoadGUI()
        {
            ...
            //ajout max
            isSectionButtons = Settings.Default.setMenu_isSectionButtons;
            isUturn_without_Autosteer = Settings.Default.setMenu_isUturn_without_Autosteer;
            if (yt.Alternate_skips == true) { cboxpRowWidth.Enabled = false; }
            //fin
        ...
        public void LineUpManualBtns()
        {
            ...
            if (isSectionButtons)
            {
                ...
                int top = 210;
                if (panelSim.Visible == true) top = 210;
                ...
                int oglButtonWidth = oglMain.Width * 2 / 3;
                //if (tool.numOfSections < 9 )  oglButtonWidth = oglMain.Width * 5/6;

                int buttonMaxWidth = 200, buttonHeight = 30;
