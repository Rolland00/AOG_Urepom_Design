...
namespace AgOpenGPS
{
    ...
    public partial class FormGPS : Form
    {
        ...
        //ajout max
        public SoundPlayer sndON;
        public SoundPlayer sndOFF;
        ...
        public FormGPS()
        {
            ...
            label4.Text = gStr.gsApplied;
            label5.Text = gStr.gsRemaining;
            sonToolStripMenuItem.Text = gStr.gsSound;
            //NTRIP
        ...
        private void FormGPS_Load(object sender, EventArgs e)
        {
            ...
            string ONwave = Path.Combine(directoryName, "Dependencies\\Audio", "ON.Wav");
            string OFFwave = Path.Combine(directoryName, "Dependencies\\Audio", "OFF.Wav");
            if (File.Exists(wave))
            {
            ...
            if (File.Exists(ONwave))
            {
                sndON = new SoundPlayer(ONwave);

            }
            else
            {
                sndON = new SoundPlayer(Properties.Resources.ON);
            }
            if (File.Exists(OFFwave))
            {
                sndOFF = new SoundPlayer(OFFwave);

            }
            else
            {
                sndOFF = new SoundPlayer(Properties.Resources.OFF);
            }
            // ajout max fin
            ...
            //ajout max
            camera.zoomValue = camera.zoomValue - 7;
            camera.camSetDistance = camera.zoomValue * camera.zoomValue * -1;
            SetZoom();
        ...
        //ajout max
        public void Alternate_skip_On()
        {
            if (yt.Alternate_skips == true)
            {
                yt.rowSkipsWidth = yt.Skips_2;
                cboxpRowWidth.Enabled = false;
            }
            else
            {
        ...
        private void stripSectionColor_Click(object sender, EventArgs e)
        {
            ...
            Color_button.BackColor = sectionColorDay;
        }
        ...
        private void btnDayNightMode_Click_1(object sender, EventArgs e)
        {
            SwapDayNightMode();
        }
        ...
        {
            if (Settings.Default.Son_actif == 1)
            {
                Settings.Default.Son_actif = 0;
                Settings.Default.Save();
                sonToolStripMenuItem.Checked = false;
            }
            else
            {
                Settings.Default.Son_actif = 1;
                Settings.Default.Save();
                sonToolStripMenuItem.Checked = true;
            }
        }
        ...
        {
            DialogResult result3 = MessageBox.Show("Voulez-vous vraiment quitter l'application ?",
                    "Quitter AgOpenGPS",
        ...
        //ajout max
        private void sonToolStripMenuItem_Click_1(object sender, EventArgs e)
        {
            if (Settings.Default.Son_actif == 1)
            {
                Settings.Default.Son_actif = 0;
        ...
        //fin
        private void lblSpeed_Click_2(object sender, EventArgs e)
        {
        ...
        private void vehicleToolStripBtn_Click_1(object sender, EventArgs e)
        {
            using (var form = new FormSettings(this, 0))
            {
                var result = form.ShowDialog();
        ...
        private void btnAutoYouTurn_MouseClick(object sender, MouseEventArgs e)
        {

        }

        private void distanceToolBtn_Click(object sender, EventArgs e)
        {
            fd.distanceUser = 0;
